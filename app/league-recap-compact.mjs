...
export function drawCompactRecap(pdf,story,{primary=[19,67,161],accent=[255,190,38],deep=[6,17,39],logo,portraits={},icons={}}={},cards=[]){
  ...
  const colors={QB:[244,82,148],RB:[44,211,174],WR:[72,165,255],TE:[255,176,66],DEF:[179,133,255],K:[249,218,75]};
  const tint=color=>deep.map((v,i)=>Math.round(v*.72+color[i]*.28));
  const icon=(name,x,y,size=22)=>{if(icons[name])pdf.addImage(icons[name],'PNG',x,y,size,size);};
  ...
  const page=(title,n)=>{
    ...
    text('Emoji graphics: Twemoji / CC BY 4.0',M,770,7,muted);pdf.link(M,762,220,13,{url:'https://github.com/jdecked/twemoji'});
  };
  const visual=story.recap.visual??{starters:[],bench:[],efficiency:[],standings:[]};
  page('THE WEEK IN FOCUS',1);
  panel(M,122,548,80,primary);icon('trophy',W-190,148,32);text('HIGH-SCORE HONORS',M+14,141,10,accent,true);
  ...
  for(const [title,players] of [['IMPACT STARTERS',visual.starters],['BENCH FIREPOWER',visual.bench]]){
    ...
    shown.forEach((p,i)=>{
      ...
      const color=colors[p.position]??accent;
      panel(x,top,178,105,tint(color));
      pdf.setDrawColor(...color);pdf.setLineWidth(2);pdf.line(x+8,top+1,x+170,top+1);
      pdf.setFillColor(...primary);pdf.circle(x+27,top+27,19,'F');pdf.setDrawColor(...color);pdf.circle(x+27,top+27,20,'S');
      ...
      text(p.points.toFixed(2),x+58,top+27,20,white,true);text(`${p.position} / ${p.nflTeam||'NFL'}${p.shared?' / TIED':''}`,x+58,top+42,8,color,true);
  ...
  awards.forEach((a,i)=>{
    ...
    const mood=/LOSS|LUCK|WRONG|HEARTBREAK/i.test(a.label)?'cry':/FIRE|SHOOT|ARMY/i.test(a.label)?'fire':/BOUNCE|UPSET|SLAYER|BACK|STREAK/i.test(a.label)?'rocket':/PERFECT|FINISH|ENOUGH|EFFORT/i.test(a.label)?'target':/MVP|KING/i.test(a.label)?'crown':'trophy';
    const color=Object.values(colors)[i%6];
    panel(x,y,178,height,tint(color));pdf.setDrawColor(...color);pdf.setLineWidth(2);pdf.line(x+7,y+1,x+171,y+1);
    icon(mood,x+146,y+8,22);
    lines(a.label,x+10,y+16,130,8.5,2,color,true);
  ...
  standings.forEach((t,i)=>{
    ...
    if(t.rank<=3)icon(t.rank===1?'crown':'trophy',x+1,y+2,20);else text(t.rank,x+4,y+15,11,accent,true);
