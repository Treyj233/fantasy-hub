...
type DraftSettings = { teams: number; slot: number; format: "Redraft" | "Keeper" | "Dynasty"; lineup: "1QB" | "Superflex"; scoring: "Standard" | "Half PPR" | "Full PPR" | "TE Premium"; cpu: "Balanced" | "Aggressive" | "Chaotic" | "Competitive"; timer: number; roster: RosterConfig };
...
const cpuScore = (player: DraftPlayer, profile: CpuProfile, teamPicks: Pick[], overall: number, settings: DraftSettings) => {
  ...
  const volatility = settings.cpu === "Competitive" ? 2 : settings.cpu === "Chaotic" ? 22 : settings.cpu === "Aggressive" ? 12 : 6;
...
export default function DraftDashboard({ players, isPro, isElite, onUpgrade }: { players: DraftPlayer[]; isPro: boolean; isElite: boolean; onUpgrade: () => void }) {
  ...
  useEffect(() => {
    ...
    const delay = settings.cpu === "Aggressive" ? 180 : settings.cpu === "Chaotic" ? 420 : settings.cpu === "Competitive" ? 220 : 280;
  ...
  return <div className="page-content draft-hq-page">
    ...
    {showSetup && <section className="draft-setup panel"><header><div><span>ROOM SETTINGS</span><h3>Make this mock yours.</h3></div><button type="button" onClick={() => setShowSetup(false)}>Collapse</button></header><div className={!isPro ? "draft-settings-grid gated" : "draft-settings-grid"}>
      ...
      <label>Rounds<select disabled value={rounds}><option>{rounds}</option></select><small>Calculated from roster</small></label>
      ...
      <label>CPU behavior<select disabled={!isElite} value={settings.cpu} onChange={(event) => setSettings({...settings, cpu:event.target.value as DraftSettings["cpu"]})}>{["Balanced","Aggressive","Chaotic","Competitive"].map((value)=><option key={value}>{value}</option>)}</select><small>{!isElite ? "Elite control" : settings.cpu === "Competitive" ? "Tight ADP discipline · no Round 1 QBs" : "Randomized room personalities"}</small></label>
      {(["QB","RB","WR","TE","FLEX"] as const).map((rosterPosition)=><label key={rosterPosition}>{rosterPosition} spots<select disabled={!isPro} value={settings.roster[rosterPosition]} onChange={(event)=>setRosterCount(rosterPosition,Number(event.target.value))}>{Array.from({length:rosterPosition==="QB"?2:5},(_,index)=>rosterPosition==="QB"?index+1:index).map((value)=><option key={value}>{value}</option>)}</select></label>)}
      {settings.lineup === "Superflex" && <label>Superflex spots<select disabled value={1}><option>1</option></select><small>Included by format</small></label>}
      <label>Bench spots<select disabled={!isPro} value={settings.roster.BENCH} onChange={(event)=>setRosterCount("BENCH",Number(event.target.value))}>{Array.from({length:13},(_,index)=><option key={index}>{index}</option>)}</select></label>
    </div>{!isPro && <div className="draft-inline-gate"><b>PRO</b><span>Unlock custom teams, rounds, formats, scoring, and draft position.</span><button onClick={onUpgrade}>View plans</button></div>}<footer><small>{players.length >= 40 ? "Using your league-adjusted rankings" : "Previewing with the Fantasy Hub demo player pool"}</small><button className="draft-primary" onClick={start}>{started ? "Restart Mock Draft" : "Start Mock Draft"} <b>→</b></button></footer></section>}
