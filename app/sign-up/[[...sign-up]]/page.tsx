...
export default async function SignUpPage({ searchParams }: { searchParams: Promise<{ native?: string }> }) {
  ...
  const emailSignUp = <SignUp routing="path" path="/sign-up" signInUrl={nativeIos ? "/sign-in?native=ios" : "/sign-in"} forceRedirectUrl={nativeIos ? "/native-auth-return" : "/"} appearance={nativeIos ? nativeEmailOnlyClerkAppearance : chargersClerkAppearance} />;
