...
type LeagueTeam = { id: string; ownerId?: string; managerName: string; teamName: string; matchupId?: number | null; roster: Player[]; draftCapital?: { score: number; picks: DraftPick[] } };
...
export default function FantasyHub({ accountUser }: { accountUser: AccountUser | null }) {
  ...
  const [leagueSeason, setLeagueSeason] = useState(String(new Date().getFullYear()));
  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <div className="league-card"><span>ACTIVE LEAGUE</span><strong>{leagueName}</strong><small>{selectedLeagueTeam ? `${selectedLeagueTeam.teamName} · ` : ""}{rankingContext?.scoring ?? "Scoring pending"} · {periodLabel}</small></div>
      ...
      <section className="workspace">
        ...
        {view === "Start / Sit" && (rosterReady ? <StartSit players={players} teams={leagueTeams} selectedTeamId={selectedTeamId} choice={starterChoice} setChoice={setStarterChoice} context={rankingContext} /> : rosterEmptyState)}
...
function NflGames({ leagueId, season, defaultWeek }: { leagueId: string; season: string; defaultWeek: number }) {
  const [week, setWeek] = useState(defaultWeek);
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      try {
        ...
        try {
          const scheduleResponse = await fetch(`/api/nfl-schedule?season=${encodeURIComponent(season)}`);
          const schedule = await scheduleResponse.json() as NflScheduleData & { error?: string };
          if (!scheduleResponse.ok) throw new Error(schedule.error ?? "Schedule unavailable");
          const selectedWeek = schedule.weeks.find((item) => item.week === week) ?? schedule.weeks[0];
          const games = (selectedWeek?.games ?? []).map((game) => ({
            id: game.id, date: game.date, name: `${game.away.name} at ${game.home.name}`, status: game.status || "Scheduled", state: "pre", clock: "", venue: "", broadcast: game.broadcast,
            teams: [{ ...game.away, displayName: game.away.name, homeAway: "away", score: 0, winner: false, color: nflThemes.find((team) => team.id === normalizeNflTeam(game.away.abbreviation))?.primary.slice(1) ?? "4b5563", logo: null, record: "" }, { ...game.home, displayName: game.home.name, homeAway: "home", score: 0, winner: false, color: nflThemes.find((team) => team.id === normalizeNflTeam(game.home.abbreviation))?.primary.slice(1) ?? "4b5563", logo: null, record: "" }],
            impactPlayers: [],
          }));
          if (!active) return;
          setData({ league: { name: "NFL Schedule", season: String(schedule.season) }, week: selectedWeek?.week ?? week, updatedAt: schedule.updatedAt, scoresAvailable: false, fallbackSchedule: true, fantasyMatchup: { available: false, yourPoints: 0, opponentPoints: 0, opponentName: "Opponent", playerCount: 0 }, games });
          setError("");
        } catch {
  ...
  return <div className="page-content nfl-games-page"><section className="nfl-games-head"><div><span>NFL GAME HUB</span><h2>Every game. Your matchup in focus.</h2><p>Live NFL scores refresh every 30 seconds. Matchup players are attached when your fantasy matchup has been posted.</p></div><label>Week<select value={week ?? ""} onChange={(event) => setWeek(Number(event.target.value))}>{Array.from({ length: 18 }, (_, index) => index + 1).map((value) => <option key={value} value={value}>Week {value}</option>)}</select></label><div className="live-refresh"><i />{loading ? "Refreshing" : `Updated ${data ? new Date(data.updatedAt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }) : "—"}`}</div></section>{data?.fallbackSchedule ? <section className="schedule-fallback"><b>SCHEDULE MODE</b><span>Live scores are temporarily unavailable. Fantasy Hub is showing the saved NFL schedule and will restore live game data automatically.</span></section> : data?.fantasyMatchup.available ? <section className="fantasy-score-ribbon"><span>YOUR FANTASY MATCHUP</span><strong>You {data.fantasyMatchup.yourPoints.toFixed(2)}</strong><i>vs</i><strong>{data.fantasyMatchup.opponentName} {data.fantasyMatchup.opponentPoints.toFixed(2)}</strong><small>{data.fantasyMatchup.playerCount} players mapped to NFL games</small></section> : data && <section className="fantasy-matchup-pending">Fantasy matchup details have not been posted for Week {data.week}. The complete NFL slate is still available below.</section>}{error && <section className="scoreboard-error">{error}</section>}<div className="nfl-game-grid">{data?.games.map((game) => <article className={`nfl-game-card ${game.impactPlayers.length ? "has-impact" : ""}`} key={game.id}><header><div><span className={game.state === "in" ? "game-live" : ""}>{game.state === "in" ? "● LIVE" : game.status}</span>{game.clock && <b>{game.clock}</b>}</div><small>{game.state === "pre" ? new Date(game.date).toLocaleString([], { weekday: "short", hour: "numeric", minute: "2-digit" }) : game.broadcast || game.venue}</small>{game.impactPlayers.length > 0 && <em>{game.impactPlayers.length} MATCHUP PLAYERS</em>}</header><div className="nfl-score-bug">{game.teams.map((team) => <div key={team.abbreviation}><span style={{ backgroundColor: `#${team.color}` }}>{team.abbreviation}</span><p><strong>{team.displayName}</strong><small>{team.record}{team.homeAway === "home" ? " · HOME" : ""}</small></p><b>{data.scoresAvailable === false ? "—" : team.score}</b>{team.winner && <i>▲</i>}</div>)}</div>{game.impactPlayers.length > 0 ? <section className="impact-roster"><h4>Players in your fantasy matchup</h4>{game.impactPlayers.map((player) => <div className={player.side === "You" ? "your-player" : "opponent-player"} key={`${player.side}-${player.id}`}><span className={`pos pos-${player.position.toLowerCase()}`}>{player.position}</span><p><strong>{player.name}</strong><small>{player.nflTeam} · {player.starter ? "Starter" : "Bench"}</small></p><em>{player.side}</em><b>{player.fantasyPoints.toFixed(1)}<small>PTS</small></b></div>)}</section> : <p className="no-impact">{data.fallbackSchedule ? "Live matchup-player tracking will return with the score feed." : "Fantasy matchup highlighting will appear here after the league posts this week’s matchup."}</p>}</article>)}</div>{data && !data.games.length && <section className="panel scoreboard-empty">No regular-season NFL games were returned for Week {data.week}.</section>}</div>;
}
...
function CommandCenter({ players, waiverPlayers, totals, setView, setSelectedPlayer, starterChoice, setStarterChoice, periodLabel }: { players: Player[]; waiverPlayers: WaiverPlayer[]; totals: { projection: number; ceiling: number }; setView: (v: View) => void; setSelectedPlayer: (p: Player) => void; starterChoice: string; setStarterChoice: (v: string) => void; periodLabel: string }) {
  ...
  return <div className="page-content">
    <section className="hero"><div><p>LINEUP LOCK · GAME DAY HQ</p><h2>{periodLabel === "PRESEASON" ? "Build the lineup" : "Let’s go win"}<br /><em>{periodLabel === "PRESEASON" ? "before Week 1." : `${periodLabel}.`}</em></h2><span>Your roster is in the mix. One smart lineup call and an early waiver swing can turn a good week into a statement win.</span><div className="game-day-pills"><b>🔥 Roster ready</b><b>⚡ Lineup edges</b><b>🎯 League-specific model</b></div></div><div className="hero-score"><small>YOU’RE PROJECTED FOR</small><strong>{totals.projection.toFixed(1)}</strong><span>Ceiling {totals.ceiling.toFixed(1)} · Let it fly</span></div></section>
...
function MyTeam({ players, setSelectedPlayer }: { players: Player[]; setSelectedPlayer: (p: Player) => void }) {
  ...
  return <div className="page-content"><SectionIntro kicker="ROSTER CONTROL" title="Your complete roster in league order" text="League projection preserves the host site’s weekly forecast under your scoring. Fantasy Hub independently adjusts role, opportunity, availability, confidence, and weather." /><RosterSection title="Starters" detail={`${starters.length} active lineup slots`} players={starters} setSelectedPlayer={setSelectedPlayer} /><RosterSection title="Reserves" detail={`${reserves.length} bench, IR, and taxi players`} players={reserves} setSelectedPlayer={setSelectedPlayer} /></div>;
}
...
function StartSit({ players, teams, selectedTeamId, choice, setChoice, context }: { players: Player[]; teams: LeagueTeam[]; selectedTeamId: string; choice: string; setChoice: (v: string) => void; context: RankingContext | null }) {
  ...
  return <div className="page-content"><SectionIntro kicker="WEEKLY DECISIONS" title="Choose the outcome your matchup requires" text={context ? `Every projection uses this league’s ${context.scoring} scoring and ${context.scoringRuleCount} active scoring rules before floor and ceiling are weighted for matchup posture.` : "A favorite should protect its floor. An underdog may need more volatility and ceiling to create a realistic path to win."} />{context && <section className="ranking-context start-sit-scoring"><span><b>{context.scoring}</b> receptions</span><span><b>{context.passTouchdown} pts</b> passing TD</span><span><b>{context.interception} pts</b> interceptions</span>{context.tePremium > 0 && <span><b>+{context.tePremium} per TE catch</b> TE premium</span>}{context.bonusRuleCount > 0 && <span><b>{context.bonusRuleCount}</b> bonus rules</span>}</section>}
    <section className="projection-source panel"><div><span>PROJECTION SOURCE</span><strong>{projectionSource === "hub" ? "Fantasy Hub model" : "League platform"}</strong><small>The selected source powers team totals, player ranges, and the recommendation.</small></div><div role="group" aria-label="Start sit projection source"><button className={projectionSource === "hub" ? "active" : ""} onClick={() => setProjectionSource("hub")}>Fantasy Hub</button><button className={projectionSource === "league" ? "active" : ""} onClick={() => setProjectionSource("league")}>League projections</button></div></section>
    <section className="risk-console panel"><div className="matchup-inputs"><label>{yourTeam?.teamName ?? "Your team"}<input type="number" step="0.1" value={teamProjection.toFixed(1)} readOnly /></label><span>VS</span><label>{opponentTeam?.teamName ?? "Opponent not posted"}<input type="number" step="0.1" value={opponentProjection?.toFixed(1) ?? ""} placeholder="—" readOnly /></label></div><div className="risk-recommendation"><span>RECOMMENDED APPROACH</span><strong>{recommendedAggression}% · {recommendedAggression < 35 ? "Play it safe" : recommendedAggression > 65 ? "Shoot for upside" : "Balanced"}</strong><p>{opponentProjection == null ? "The league has not posted a weekly opponent yet, so Fantasy Hub defaults to a balanced posture without inventing a matchup total." : matchupGap > 3 ? `You project ${matchupGap.toFixed(1)} points behind. Accept more variance to improve your upset path.` : matchupGap < -3 ? `You project ${Math.abs(matchupGap).toFixed(1)} points ahead. Protect the favorite outcome with dependable volume.` : "The matchup is close enough to favor balanced median outcomes."}</p><button onClick={() => setAggressiveness(recommendedAggression)}>Use recommended</button></div></section>
