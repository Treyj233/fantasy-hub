...
function ManagerReport({ leagueId }: { leagueId: string }) {
  ...
  return <div className="page-content manager-report-page">
    ...
    <section className="manager-weekly-grid">
      ...
      <section className="manager-activity manager-trades panel"><header><div><span>COMPLETED TRADES</span><h3>Deals that actually processed</h3></div><small>Week {data.league.week}</small></header><div className="manager-activity-scroll">{data.observed.trades.length ? data.observed.trades.map((trade) => <article key={trade.id}><b>↔</b><div><strong>Received: {assetList(trade.received, trade.picksReceived)}</strong><small>Sent: {assetList(trade.sent, trade.picksSent)}</small></div><em>Completed<small>{activityTime(trade.timestamp)}</small></em></article>) : <p className="story-empty">No completed Sleeper trades involving your roster this week.</p>}</div></section>
    </section>
