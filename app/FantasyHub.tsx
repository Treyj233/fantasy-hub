...
function LeagueStories({ leagueId, setView }: { leagueId: string; setView: (view: View) => void }) {
  ...
  return <><div className="page-content league-stories-page">
    ...
    <div className="story-dashboard-grid">
      ...
      <section className="panel trade-reactions"><header><span>TRADE WIRE</span><h3>Completed deals</h3></header>{story.trades.length ? story.trades.map((trade) => { const text = `Week ${trade.week} trade in ${story.league.name}: ${trade.adds.map((item) => `${item.player} to ${item.team}`).join(", ")}.`; return <article key={trade.id}><div className="trade-wire-top"><b>WEEK {trade.week}</b><button onClick={() => void shareStory(trade.id, text)}>{shared === trade.id ? "Copied!" : "Share"}</button></div><strong className="trade-wire-teams">{trade.teams.join(" ↔ ")}</strong><div className="trade-wire-assets">{trade.adds.map((item) => <span key={`${trade.id}-${item.player}-${item.team}`}><strong>{item.player}</strong><small>→ {item.team}</small></span>)}</div></article>; }) : <p className="story-empty">No completed trades were observed in the current recap window.</p>}</section>
    </div>
