  ...
  return <div className="page-content theme-store-page">
    ...
    {tab === "library" ? <section className="appearance-panel theme-store-catalog panel" role="tabpanel">
      ...
      <section className="pro-theme-collections"><header><span>INCLUDED WITH PRO</span><h4>Two complete starter collections.</h4><p>Upgrade once and both packs appear automatically in My Library.</p></header><div>
        <article className="pro-collection-card nfl-collection"><div className="pro-collection-art" aria-hidden="true">{proNflThemes.slice(0,8).map((theme)=><i key={theme.id} style={{background:`linear-gradient(135deg,${theme.primary} 0 50%,${theme.secondary} 50%)`}}><b>{theme.shortCode ?? theme.id}</b></i>)}</div><em>32 TEAM-INSPIRED THEMES</em><h5>Team-Inspired Theme Collection</h5><p>Every current team-inspired dashboard palette, packaged together and ready to use across Fantasy Hub.</p><div className="theme-card-actions">{!isPro && <button className="theme-preview-trigger" onClick={()=>{setPreviewThemeId(proNflThemes[0]?.id??"");setPreviewScene("home")}}>PREVIEW THEMES</button>}<button disabled={isPro} onClick={onUpgrade}>{isPro ? "PRO · IN YOUR LIBRARY" : "GET PRO ACCESS →"}</button></div></article>
    ...
    {previewThemeId&&previewSelection&&<div className="theme-preview-backdrop" role="presentation" onMouseDown={(event)=>{if(event.target===event.currentTarget)setPreviewThemeId("")}}><section className="theme-preview-modal" role="dialog" aria-modal="true" aria-labelledby="theme-preview-title" style={{"--preview-primary":previewSelection.theme.primary,"--preview-secondary":previewSelection.theme.secondary} as CSSProperties}><header><div><span>NONINTERACTIVE THEME PREVIEW</span><h3 id="theme-preview-title">{previewSelection.theme.name}</h3><small>{previewSelection.badge.name} badges · Your app theme remains unchanged</small></div><button type="button" aria-label="Close theme preview" onClick={()=>setPreviewThemeId("")}>×</button></header><nav className="theme-preview-scenes" aria-label="Preview screens">{([['home','Home'],['gameday','Gameday Live'],['trade','Trade Lab']] as const).map(([id,label])=><button type="button" key={id} className={previewScene===id?"active":""} aria-pressed={previewScene===id} onClick={()=>setPreviewScene(id)}>{label}</button>)}</nav><div className={`theme-preview-stage scene-${previewScene}`} aria-label={`${previewSelection.theme.name} ${previewScene} preview`}><aside><b>FH</b><i>⌂</i><i>⚡</i><i>↔</i><i>★</i></aside><main><header><span><b>Fantasy Hub</b><small>{previewScene==='home'?'Mission Hub':previewScene==='gameday'?'Sunday Pulse':'Trade Intelligence'}</small></span><em>LIVE</em></header>{previewScene==='home'?<div className="theme-preview-home"><section><span>PRIORITIZED INBOX</span><strong>Your leagues, under control.</strong><article><b>1</b><p><strong>Set your WR2 before kickoff</strong><small>Sunday Legends · Lineup</small></p></article><article><b>2</b><p><strong>Monitor Josh Allen status</strong><small>Dynasty North · Availability</small></p></article></section><aside><span>WEEKLY READINESS</span><strong>86</strong><i><b/></i><small>6 of 7 lineups ready</small></aside></div>:previewScene==='gameday'?<div className="theme-preview-gameday"><section><span>SUNDAY PULSE</span><strong>3 games impacting your teams</strong></section><div><article><small>BUF · 3RD</small><strong>Josh Allen</strong><b>21.8 PTS</b></article><article><small>MIA · RED ZONE</small><strong>De'Von Achane</strong><b>🔥 HOT</b></article><article><small>PHI · FINAL</small><strong>A.J. Brown</strong><b>18.4 PTS</b></article></div></div>:<div className="theme-preview-trade"><section><span>YOU SEND</span><strong>CeeDee Lamb</strong><small>WR · DAL</small><b>91</b></section><i>↔</i><section><span>YOU RECEIVE</span><strong>Garrett Wilson</strong><small>WR · NYJ</small><b>88</b></section><footer><strong>FAIR TRADE</strong><small>3% adjusted value gap</small></footer></div>}</main></div><div className="theme-preview-picker"><button type="button" aria-label="Previous theme" onClick={()=>{const index=previewThemes.findIndex(({theme})=>theme.id===previewSelection.theme.id);setPreviewThemeId(previewThemes[(index-1+previewThemes.length)%previewThemes.length].theme.id)}}>‹</button><div>{previewThemes.map(({theme})=><button type="button" key={theme.id} className={theme.id===previewSelection.theme.id?"active":""} aria-label={`Preview ${theme.name}`} aria-pressed={theme.id===previewSelection.theme.id} style={{background:`linear-gradient(135deg,${theme.primary} 0 50%,${theme.secondary} 50%)`}} onClick={()=>setPreviewThemeId(theme.id)}/>)}</div><button type="button" aria-label="Next theme" onClick={()=>{const index=previewThemes.findIndex(({theme})=>theme.id===previewSelection.theme.id);setPreviewThemeId(previewThemes[(index+1)%previewThemes.length].theme.id)}}>›</button></div><footer><span><b>{previewSelection.theme.name}</b><small>Preview only · The dashboard cannot be operated</small></span><button type="button" onClick={()=>{setPreviewThemeId("");if(previewSelection.kind==='pro')onUpgrade();else void addPremiumBundle(previewSelection.theme.id,previewSelection.badge.id,previewSelection.productId)}}>{previewSelection.kind==='pro'?"UNLOCK WITH PRO →":isElite?"CLAIM WITH ELITE →":nativeIos?`UNLOCK ${packPrices[previewSelection.productId]??"$1.99"} →`:"VIEW PURCHASE OPTIONS →"}</button></footer></section></div>}
  </div>;
}
