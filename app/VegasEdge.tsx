...
export default function VegasEdge({roster,waivers,context,season,week,onPlayer,onWaivers,enabled,onToggle,onFeed}:{roster:EdgePlayer[];waivers:EdgePlayer[];context:EdgeContext;season:string;week:number;onPlayer:(p:EdgePlayer)=>void;onWaivers:()=>void;enabled:boolean;onToggle:(on:boolean)=>void;onFeed:(feed:EdgeFeed)=>void}) {
  ...
  return <div className="page-content vegas-edge-page">
    ...
    <details className="panel edge-method"><summary>Projection &amp; market details</summary><dl><div><dt>Vegas estimate</dt><dd>60% market-derived + 40% platform. An estimate, not a sportsbook fantasy total.</dd></div><div><dt>Coverage</dt><dd>Missing or stale lines fall back to platform projections. Only fresh, eligible players generate moves.</dd></div><div><dt>Scoring</dt><dd>Uses your league settings. TDs and interceptions are odds-adjusted; bonuses and secondary stats may be incomplete.</dd></div><div><dt>Refresh</dt><dd>Shared checks on page open or refresh. Frequency increases near kickoff; no unattended refresh.</dd></div></dl>{feed?.message&&<p>{feed.message}</p>}{feed?.budget&&<small>Market usage: {feed.usage ?? '—'} / {feed.budget}</small>}</details>
  </div>;
}
