...
export default function VegasEdge({roster,waivers,context,season,week,onPlayer,onWaivers,enabled,onToggle,onFeed,renderRosterColumns}:{renderRosterColumns:(p:EdgePlayer)=>ReactNode;roster:EdgePlayer[];waivers:EdgePlayer[];context:EdgeContext;season:string;week:number;onPlayer:(p:EdgePlayer)=>void;onWaivers:()=>void;enabled:boolean;onToggle:(on:boolean)=>void;onFeed:(feed:EdgeFeed)=>void}) {
  ...
  return <div className="page-content vegas-edge-page">
    ...
    <section className="panel edge-board"><header><div><h3>Roster outlook</h3><p>Week {week} · {context.scoring} · Pregame</p></div><div className="edge-filters" aria-label="Roster filters">{['All','Starters','Bench','Covered'].map(f=><button key={f} aria-pressed={filter===f} onClick={()=>setFilter(f)}>{f}</button>)}</div></header>
      <div className="edge-table-wrap"><table><colgroup><col className="edge-name-col"/><col className="edge-slot-col"/><col className="edge-matchup-col"/><col/><col/><col/></colgroup><thead><tr><th>Player</th><th>Slot</th><th>Matchup</th><th>Platform</th><th>Vegas</th><th>Edge</th></tr></thead><tbody>{visible.map(r=><tr key={r.player.id}>{renderRosterColumns(r.player)}<td>{r.baseline.toFixed(1)}</td><td className="edge-projection">{r.projection?.toFixed(1) ?? '—'}<small className="edge-coverage-label">{r.label}</small>{r.props.length>0&&<button type="button" className="edge-market-trigger" aria-haspopup="dialog" aria-label={`View prop markets for ${r.player.name}`} onClick={()=>setMarketPlayer(r.player.id)}>{r.props.length} props ↗</button>}</td><td data-direction={(r.delta ?? 0)>0?'up':(r.delta ?? 0)<0?'down':'flat'}>{r.delta===null?'—':signed(r.delta)}</td></tr>)}</tbody></table></div>
