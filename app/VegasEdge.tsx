...
export default function VegasEdge({roster,waivers,context,season,week,onPlayer,onWaivers,enabled,onToggle,onFeed}:{roster:EdgePlayer[];waivers:EdgePlayer[];context:EdgeContext;season:string;week:number;onPlayer:(p:EdgePlayer)=>void;onWaivers:()=>void;enabled:boolean;onToggle:(on:boolean)=>void;onFeed:(feed:EdgeFeed)=>void}) {
  ...
  return <div className="page-content vegas-edge-page">
    ...
    <section className="panel edge-board"><header><div><h3>Roster outlook</h3><p>Week {week} · {context.scoring} · Pregame</p></div><div className="edge-filters" aria-label="Roster filters">{['All','Starters','Bench','Covered'].map(f=><button key={f} aria-pressed={filter===f} onClick={()=>setFilter(f)}>{f}</button>)}</div></header>
      <div className="edge-table-wrap"><table><colgroup><col className="edge-name-col"/><col/><col/><col/></colgroup><thead><tr><th>Player</th><th>Platform</th><th>Vegas</th><th>Edge</th></tr></thead><tbody>{visible.map(r=><tr key={r.player.id}><td><button className="edge-player" onClick={()=>onPlayer(r.player)}>{r.player.name}</button><small>{r.player.position} · {r.player.team} · {r.player.role}</small><span className={`edge-status ${r.usable?'is-covered':''}`}>{r.label}{r.questionable?' · Questionable':''}</span>{r.props.length>0&&<button type="button" className="edge-market-trigger" aria-haspopup="dialog" aria-label={`View prop markets for ${r.player.name}`} onClick={()=>setMarketPlayer(r.player.id)}>{r.props.length} props ↗</button>}</td><td>{r.baseline.toFixed(1)}</td><td className="edge-projection">{r.projection?.toFixed(1) ?? '—'}</td><td data-direction={(r.delta ?? 0)>0?'up':(r.delta ?? 0)<0?'down':'flat'}>{r.delta===null?'—':signed(r.delta)}</td></tr>)}</tbody></table></div>
