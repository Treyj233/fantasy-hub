...
function PreviewCards({children,className,label,limit=2}:{children:ReactNode;className:string;label:string;limit?:number}){
  const [expanded,setExpanded]=useState(false);
  const id=useId();
  const cards=Children.toArray(children);
  return <><div className={className} id={id}>{expanded?cards:cards.slice(0,limit)}</div>{cards.length>limit&&<button type="button" className="lw-expand" aria-expanded={expanded} aria-controls={id} onClick={()=>setExpanded(value=>!value)}>{expanded?'Show less':`Show all ${cards.length} ${label}`} <span aria-hidden="true">{expanded?'−':'+'}</span></button>}</>;
}
function Performers({title,players}:{title:string;players:Performer[]}){
  return <section className="lw-section"><header><span>POSITION SPOTLIGHTS</span><h3>{title}</h3></header>{players.length?<PreviewCards className="lw-performers" label="players" limit={2}>{players.map(p=><article key={`${p.rosterId}:${p.id}`}><div className="lw-player-top"><span className="lw-position">{p.position}</span><Identity src={p.image} name={p.name}/></div><h4>{p.name}</h4><span className="lw-nfl">{p.nflTeam}{p.shared?' · Shared honors':''}</span><strong className="lw-player-score">{p.points.toFixed(2)}<small>PTS</small></strong><footer>{p.teamName}</footer></article>)}</PreviewCards>:<p className="lw-empty">No confirmed player scoring available for this group.</p>}</section>;
}
export default function LeagueWeeklyReport({story,onShare,shared}:{story:LeagueStoryData;onShare:()=>void;shared:boolean}){
  ...
  return <section className="lw-report" aria-label={`Week ${recap.week} league report`}>
    ...
    {!recap.available?<p className="lw-empty">Your weekly report will appear once completed matchup scores are available.</p>:<>
      ...
      <section className="lw-section" id="lw-honors"><header><span>THE BRAGGING RIGHTS</span><h3>Fantasy Hub honors</h3></header><PreviewCards className="lw-honors" label="awards" limit={2}>{awards.map((award,index)=>{const metric=award.detail.match(/^([+\-]?\d+(?:\.\d+)?%?)/)?.[1];return <article key={award.id}><header><b>{String(index+1).padStart(2,'0')}</b><span>{award.label}</span></header><h4>{award.recipient}</h4>{metric&&<strong className="lw-award-value">{metric}</strong>}<p>{award.detail}</p></article>;})}</PreviewCards></section>
      {visual&&<div className="lw-analysis"><section className="lw-section" id="lw-efficiency"><header><span>POINTS CAPTURED</span><h3>Lineup efficiency</h3><p>Actual starter points against your best legal lineup. A look back, not a judgment of the original decision.</p></header><PreviewCards className="lw-efficiencies" label="teams" limit={3}>{visual.efficiency.map(t=><article key={t.rosterId}><div><Identity src={t.avatar} name={t.teamName}/><h4>{t.teamName}</h4><strong>{t.percent===null?'—':`${t.percent.toFixed(1)}%`}</strong></div>{t.percent!==null?<><meter min={0} max={100} value={t.percent} aria-label={`${t.teamName}: ${t.percent.toFixed(1)} percent of optimal points`}/><p><b>{t.actual?.toFixed(2)}</b> of {t.maximum?.toFixed(2)} available points</p></>:<p>Complete lineup scoring unavailable</p>}</article>)}</PreviewCards></section>
      <section className="lw-section" id="lw-ladder"><header><span>THROUGH WEEK {recap.week}</span><h3>The league ladder</h3><p>Head-to-head record, then points scored.</p></header><PreviewCards className="lw-standings" label="teams" limit={3}>{visual.standings.map(t=><article key={t.rosterId} className={t.isMine?'is-mine':''}><span className="lw-rank">{t.rank}</span><Identity src={t.avatar} name={t.teamName}/><div className="lw-standing-team"><h4>{t.teamName}</h4><small>{t.wins}-{t.losses}{t.ties?`-${t.ties}`:''} · {t.managerName}{!t.complete?' · Partial data':''}</small></div><span className="lw-standing-points"><b>{t.pf.toFixed(2)}</b><small>PF</small></span><span className="lw-standing-points"><b>{t.pa.toFixed(2)}</b><small>PA</small></span><span className="lw-movement" aria-label={t.movement?`${Math.abs(t.movement)} places ${t.movement>0?'up':'down'}`:'No rank movement'}>{t.movement?`${t.movement>0?'↑':'↓'}${Math.abs(t.movement)}`:'—'}</span></article>)}</PreviewCards></section></div>}
    </>}
  </section>;
}
