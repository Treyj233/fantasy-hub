...
export async function normalizeEspnLeague(payload: EspnPayload, requestedWeek?: number) {
  ...
  const playerShape = (player: EspnPlayer, role = "Player pool") => {
    ...
    const currentProfile = playerSeasonProfileFor(seasonContext.currentProfiles, name);
    ...
    return { id: `espn-player:${player.id ?? 0}`, name, position, team: nflTeamById[player.proTeamId ?? 0] ?? "FA", opponent: "Matchup pending", projection: points, leagueProjection: points, floor: Number((points * .68).toFixed(1)), ceiling: Number((points * 1.38).toFixed(1)), trend: 0, status: player.injuryStatus || (player.injured ? "Questionable" : "Healthy"), role, rankingValue: Number((points * 3 + (player.ownership?.percentOwned ?? 0) * .35).toFixed(2)), ageAdjustment: 0, lineupAdjustment: 0, snapPct: snapProfile?.latestPct ?? null, snapAverage: snapProfile?.averagePct ?? null, snapWeek: snapProfile?.latestWeek ?? null, snapSeason: snapProfile?.season ?? null, statsSourceSeason: seasonContext.sourceSeason, statsBlended: seasonContext.blended || offenseContext.blended, fantasyPoints2025: historicalPoints == null ? null : Number(historicalPoints.toFixed(1)), fantasyPpg2025: seasonProfile?.games && historicalPoints != null ? Number((historicalPoints / seasonProfile.games).toFixed(1)) : null, gamesPlayed2025: seasonProfile?.games ?? null, currentSeasonGames: currentProfile?.games ?? 0, currentSeasonPpg: currentProfile?.games ? (currentProfile.fantasyPoints + currentProfile.receptions * receptionPoints) / currentProfile.games : null, targets2025: seasonProfile?.targets ?? null, receptions2025: seasonProfile?.receptions ?? null, receivingYards2025: seasonProfile?.receivingYards ?? null, receivingTouchdowns2025: seasonProfile?.receivingTouchdowns ?? null, rushingAttempts2025: seasonProfile?.rushingAttempts ?? null, rushingYards2025: seasonProfile?.rushingYards ?? null, rushingTouchdowns2025: seasonProfile?.rushingTouchdowns ?? null, passingAttempts2025: seasonProfile?.passingAttempts ?? null, passingYards2025: seasonProfile?.passingYards ?? null, passingTouchdowns2025: seasonProfile?.passingTouchdowns ?? null, team2025: seasonProfile?.team ?? null, teamOffenseRank2025: teamOffense?.rank ?? null, teamPointsPerGame2025: teamOffense?.pointsPerGame ?? null, adpBySite: { Sleeper: sleeperLeagueFormat, "Sleeper Single-QB": sleeperSingleQb, "Sleeper Superflex": sleeperSuperflex, ESPN: typeof espnAdp === "number" && espnAdp > 0 && espnAdp < 999 ? espnAdp : null, "Underdog Single-QB Half PPR": underdogSingleQbHalfPprAdp.get(sleeperKey) ?? null, "Underdog Single-QB Full PPR": underdogSingleQbFullPprAdp.get(sleeperKey) ?? null, "Underdog Superflex Half PPR": underdogSuperflexHalfPprAdp.get(sleeperKey) ?? null } };
  };
...
export function normalizeEspnScoreboard(payload: EspnPayload, ownedRosterId: string, requestedWeek?: number) {
  ...
  const teamShape = (side: { teamId?: number; totalPoints?: number } | undefined) => {
    ...
    return { rosterId: String(team?.id ?? ""), record: formatTeamRecord(team?.record?.overall), ownerId: ownerId || null, managerName: owner?.displayName ?? (`${owner?.firstName ?? ""} ${owner?.lastName ?? ""}`.trim() || "ESPN Manager"), teamName: team ? teamName(team) : "ESPN Team", points: Number((side?.totalPoints ?? topPlayers.filter((player) => player.isStarter).reduce((sum, player) => sum + player.points, 0)).toFixed(2)), isMine: String(team?.id ?? "") === ownedRosterId, topPlayers };
  };
