...
test("native sign-in clears stale Clerk state without a server redirect", async () => {
  ...
  assert.match(nativeEmailSignIn, /navigate: async \(\{ session \}\)/);
  ...
  assert.match(nativeEmailSignIn, /nativeSessionReady = true/);
  assert.match(nativeEmailSignIn, /if \(!nativeSessionReady\) throw new Error/);
