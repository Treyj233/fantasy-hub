...
test("weekly player rankings are Pro-gated, week-aware, and position limited", async () => {
  ...
  assert.match(rankings, /projectionScore \* \.65 \+ ceilingScore \* \.3 \+ matchupScore \* \.05/);
  ...
  assert.doesNotMatch(rankings, /<span>MATCHUP<\/span><strong>Opponent strength<\/strong>/);
  ...
  assert.match(styles, /grid-template-columns: 22px minmax\(0,1fr\) 30px 30px minmax\(120px,140px\) 28px;/);
