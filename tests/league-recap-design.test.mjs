...
test('recap uses the exact current iPhone launch logo, not the legacy green logo',()=>{
  ...
  const renderer=readFileSync('app/league-recap-compact.mjs','utf8');
  assert.match(renderer,/pdf\.clip\(\)/); assert.match(renderer,/pdf\.restoreGraphicsState\(\)/);
});
...
test('every award is exported with automatic page breaks and safe text bounds',()=>{
  fixture.recap.visual={
    ...
    efficiency:Array.from({length:12},(_,i)=>({rosterId:i+1,teamName:`League Team ${i+1}`,actual:120-i,maximum:150,percent:(120-i)/150*100})),
    standings:Array.from({length:12},(_,i)=>({rosterId:i+1,rank:i+1,teamName:`League Team ${i+1}`,wins:4,losses:0,ties:0,pf:600-i*10,pa:500,movement:0,complete:true,isMine:i===0}))
  };
  ...
  for(const label of labels) assert.ok(data.includes(label.split(' ')[0]),label);
...
test('shared winners and long names flow without truncating award content',()=>{
  ...
  assert.match(pdf.output(),/Full stories/);
