...
test("a new Clerk identity reconnects to existing Fantasy Hub account data by verified email", async () => {
  ...
  assert.match(source, /lower\(\$\{accountIdentities\.verifiedEmail\}\) = \$\{normalizedEmail\}/);
  assert.match(source, /lower\(\$\{subscriptions\.email\}\) = \$\{normalizedEmail\}/);
  assert.match(source, /emailIdentity\?\.canonicalUserId \?\? connection\?\.userId \?\? preferenceRows\[0\]\?\.userId \?\? subscriptionRows\[0\]\?\.userId/);
});
